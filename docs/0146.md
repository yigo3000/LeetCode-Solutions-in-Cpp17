...
```
* `std::list::remove`要先查找元素再删除，时间复杂度为`O(n)`。如果能使用`std::list::erase`，就能将时间复杂度优化到`O(1)`，为此令`unordered_map`保存`std::list`的迭代器，令`std::list`保存`key`和`value`即可
```cpp
class LRUCache {
public:
    LRUCache(int capacity) : n(capacity) {
...
private:
    int n;
    list<pair<int, int>> q; // 仍要保存key，在哈希表中移除时要知道队尾元素的key
